...
Карты подтягиваются из S3-совместимого хранилища, а `CardArt/` служит
фолбэком — **репозиторий собирается и запускается сразу после клонирования.**

Адрес по умолчанию задан в `ArtEndpoint.swift`. Переопределить можно
...
либо ключом `ArtBaseURL` в `Info.plist` сборки. Если объект не найден,
